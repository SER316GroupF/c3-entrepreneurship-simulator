...
/**
 * This class creates a navigation panel by creating the buttons and linking them with their screens.
 * It also supports having one level of sub-buttons to a parent button. 
 * A parent button will only show and hide sub-buttons when clicked.
 * Only one of the parent buttons can show its sub-buttons at the same time. 
 * If a parent button is clicked, the already extended sub-buttons will collapse before extending the new set of sub-buttons.
 * 
...
public class NavigationPanel extends Table
{
	 ...
	 *
	 *@param buttonName The string that will be displayed on the button.
	 *@param targetedScreen The screen that the button maps to. If null, no actionListener will be added to the button.
	 */
	...
	/**
	 *This method adds a sub button to the one of the main buttons in the navigation panel.
	 *Parent buttons will not direct to screens but instead will show and hide sub-buttons when clicked
	 *
	 *@param parentButtonName the name of the parent button which the sub-button will be under
	 *@param buttonName The string that will be displayed on the button.
	 *@param targetedScreen The screen that the button maps to. If null, no actionListener will be added to the button.
	 */
	public boolean addSubButton(String parentButtonName, String buttonName, Screen targetedScreen)
	{
		...
		if(!storedParentButtonName.equalsIgnoreCase(""))
		{
			...
			{
				LinkedHashMap<String, Screen> relativeSubButtons = subButtons.get(storedParentButtonName);
	...
	/**
	 *This method removes the button chosen from the navigation panel.
	 *When this method is called on a main parent button, it and all its children are removed.
	 *If a wrong string is passed that doesn't lead to a button, this method will do nothing.
	 *
	 *@param buttonName The name of the button that will be removed.
	 */
	...
	private void removeSubButton(String buttonName)
	{
		...
		{
			LinkedHashMap<String, Screen> subButtonsHashtable = subButtons.get(storedButtonName);
	 ...
	 *@param targetedScreen The new screen that the button will maps to. If null, the button won't have an actionListener.
	 */
	...
	private void recreateNavigationPanel()
	{
		...
		for(String buttonName : buttonNames)
		{
			...
			row();
			if(displayedCollapsibleButton.equalsIgnoreCase(buttonName) && subButtons.containsKey(buttonName))
			{
				LinkedHashMap<String, Screen> subButtonsHashtable = subButtons.get(buttonName);
				Set<String> subButtonNames = subButtonsHashtable.keySet();
				for(String subButtonName : subButtonNames)
				{
	...
	/**
	 *By default, all sub-buttons are hidden.
	 *This method show the sub-buttons for the chosen parent.
	 *If an empty string is passed, it will hide all sub-buttons
	 *
	 *@param parentButton The name of the parent button of the sub-buttons that will be shown.
	 */
	...
	/**
	 *This method will set the position of the navigation panel on the screen.
	 *The x and y coordinates passed will be at the top left point of the navigation panel.
	 *This is done so that the navigation panel will expand or contrast downward only when sub-buttons are shown or hidden.
	 *
	 *@param x relative to the screen (0 is at the left edge of the screen).
	 *@param y relative to the screen (0 is at the bottom edge of the screen).
	 */
	@Override
