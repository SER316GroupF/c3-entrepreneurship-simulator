 ...
 * ability to either market one of them or the company as a whole. The user is
 * also able to choose from different types of marketing campaigns. This is the
 * first step of a two-step wizard.
 * 
...
public class MarketingScreen implements SimulationScreen
{
	...
	/**
	 * The ProductIconListener takes in the label associated with the product
	 * icon and the product object itself and then manipulate them when the icon
	 * is clicked.
	 */
	private class ProductIconListener extends ClickListener
	{
		 ...
		 *            The product that is represented by the object that is
		 *            being listened to.
		 */
		public ProductIconListener(Label associatedLabel,
	...
	/**
	 * The marketingIconListener takes in the label associated with the campaign
	 * icon and the campaign object itself and then manipulate them when the
	 * icon is clicked.
	 */
	...
	private Table createMainTable()
	{
		...
		ScrollPane marketingGridScroll = new ScrollPane(getMarketingGrid(),
				skin);
		...
		nextButton.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y)
			{
				...
				{
					game.setScreen(new MarketingWizard(game,
							MarketingScreen.this));
				}
			}
		});
	...
	/**
	 * This method creates the the grid of products that the user can choose
	...
	private Table getProductGrid()
	{
		...
		for (int index = 0; index < products.length; index++)
		{
			...
			productIcon.addListener(new ProductIconListener(productLabel,
					products[index]));

			newProduct.add(productIcon).size(100f).row(); // TODO change to
															// dynamic size
	...
	/**
	 * Creates the the grid of marketing types icons that the user can choose
	 * from and adds them to a table.
	 * 
	 * @return The table that was created.
	 */
	private Table getMarketingGrid()
	{
		...
		marketingIcons.put("Internet Campaign",
				"images/placeholder-Computer-Icon.png");
		marketingIcons.put("Social Media",
				"images/placeholder-SocialMedia-icon.png");
		marketingIcons.put("Television and Radio",
				"images/placeholder--TV-icon.png");
		...
		for (String marketingType : marketingTypes)
		{
			...
			marketingtIcon.addListener(new MarketingIconListener(
					marketingMethodLabel, marketingType));

			newMarketingMethod.add(marketingtIcon).size(200f).row(); // TODO
																		// change
																		// to
																		// dynamic
																		// size
	...
	/**
	 * This method is a testing shell that creates different products and return
